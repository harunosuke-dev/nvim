...
`ys` の的にはテキストオブジェクトなら何でも渡せる。**mini.ai の `n`（次）/
`l`（前）もそのまま使える**ので、移動せずに離れた囲みを狙える。
...
| キー | すること |
|---|---|
| `ysin(*` | **次の**括弧の中を `*` で囲む → `aa (*bb*) cc` |
| `ysinq*` | 次の引用符の中を囲む → `aa "*bb*" cc` |
| `ysil(*` | **前の**括弧の中を囲む |
| `ys2i(*` | **2つ外側**の括弧の中を囲む → `f(*g(inner)*)` |

**課題** — 1行目の先頭で `ysin(*` を打つ。`f(` で移動してから `ysi(*` を打つのと
同じことが1手で済む。
