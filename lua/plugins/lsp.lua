...
return {
...

  {
    ...
    config = function()
      ...
      -- 行末の virtual_text は画面幅を超えると切り捨てられる。
      -- 仮想テキストなのでカーソルも入らず、長い型エラーは末尾が読めない。
      -- 標準の <C-w>d でも同じフロートが出るが、ウィンドウ操作の中に紛れていて
      -- 診断を読むキーとしては思い出しにくい
      vim.keymap.set('n', '<leader>cd', vim.diagnostic.open_float, {
        desc = '[C]ode [D]iagnostics : read the full message',
      })
      ...
      --   <leader>cd は <C-w>d の押しやすい別名。標準は潰さず残してある
      --
